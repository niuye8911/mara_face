...
void Results::saveROC(string outFile, vector<Results *> *rv) {
  ...
  for (unsigned int i = 0; i < rv->size(); i++) {
    ...
    if (r->N) {
      ...
      //      osd << (r->TPDisc / r->N) << " " << r->FP << " " <<
      //      r->scoreThreshold<< endl;
      float precision = float(r->TPDisc) / float(r->TPDisc + r->FP);
      float recall = float(r->TPDisc) / float(r->N);
      osd << recall << " " << precision << endl;
    } else {
      osc << "0 0" << endl;
